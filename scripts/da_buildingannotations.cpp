        ...
        if (Team == 0 || Team == 1) {
            if (Total > 0) {
                if (Damage > 0) {
                    ...
                    if (EnableRepairing && AnnounceRepairing[Team] && Get_Object_Type(Damager) == Team && !strcmp(Get_Player_Name(Damager), "None")) {
                        DA::Team_Color_Message_With_Team_Color(Team, "%s%s is repairing the %s.", DA::Get_Message_Prefix(), Get_Player_Name(Damager), DATranslationManager::Translate(Victim));
